...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let paymentConfig = { tarjetaHabilitada: true, transferenciaIban: 'ES00...', bizumTelefono: '+34...' };
  ...
  function renderPaymentMethods() {
    ...
    PAYMENT_OPTIONS.forEach(option => {
      const isOptionDisabled = option.value === 'tarjeta' && !paymentConfig.tarjetaHabilitada;
      ...
      label.innerHTML = `
        <div class="flex items-start gap-4">
          ...
          <div>
            ...
            ${option.value === 'transferencia bancaria' ? `<p class="mt-2 text-sm font-semibold text-slate-800">IBAN: ${paymentConfig.transferenciaIban}</p>` : ''}
            ${option.value === 'bizum' ? `<p class="mt-2 text-sm font-semibold text-slate-800">Telefono: ${paymentConfig.bizumTelefono}</p>` : ''}
          </div>
        </div>
      `;
  ...
  checkoutForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (selectedMethod === 'tarjeta' && payload.idPedido) {
        ...
        window.location.href = '/index.html'; // Or confirmacion.html
      }
    } catch (err) {
